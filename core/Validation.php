...
class Validation {
    ...
    static $message = '<div class="error"><ul>%s</ul></div>';
    static $row = '<li>%s</li>';
 ...
 */
    ...
    function get_message( $errors_array )
    {
        $tags = explode('%s', Validation::$row);
        $before = array_shift($tags);
        $after = array_shift($tags);
        
        return sprintf(Validation::$message, $before.implode("{$after}{$before}", $errors_array).$after);
